...
#define MAX_RECURSION_LEVEL 600
...
- (void) setContentMargin:(UIEdgeInsets)newContentMargin
{
...
- (void) configureDefaults
{
	...
	//_connectingLineColor = [AVHexColor colorWithHexString:@"#c8c8c8"];
	_parentChildSpacing = 20.0;
	_siblingSpacing = 20.0;
	_animatesLayout = YES;
    ...
    _minimumFrameSize = (CGSize){
        .width = _contentMargin.left+_contentMargin.right,
        .height = _contentMargin.top + _contentMargin.bottom
    };
...
- (void) updateRootSubtreeViewPositionForSize:(CGSize)rootSubtreeViewSize
{
    ...
    if ( [self resizesToFillEnclosingScrollView] )
    {
		...
		if (( [self treeGraphOrientation] == PSTreeGraphOrientationStyleHorizontal ) || ( [self treeGraphOrientation] == PSTreeGraphOrientationStyleHorizontalFlipped ))
        {
			newOrigin = CGPointMake([self contentMargin].left,
...
- (CGSize) layoutGraphIfNeeded
{
    ...
    if ([self needsGraphLayout] && [self modelRoot]) {
        ...
        CGSize minimumBoundsSize = CGSizeMake(rootSubtreeViewSize.width + margin.left+margin.right,
                                              rootSubtreeViewSize.height + margin.top + margin.bottom);
...
- (void) encodeWithCoder:(NSCoder *)encoder
{
    ...
    [encoder encodeUIEdgeInsets:_contentMargin forKey:@"contentMargin"];
...
- (id) initWithCoder:(NSCoder *)decoder
{
    ...
    if (self) {
        ...
        if ([decoder containsValueForKey:@"contentMargin"])
            _contentMargin = [decoder decodeUIEdgeInsetsForKey:@"contentMargin"];
